...
class Api {
  ...
  static Future getJadwalDokterByUnitPegawai(unit, pegawai) {
    var url = baseUrl +
        "/dokter/cari_jadwal_by_unit_pegawai?unitid=" +
        unit +
        "&pegawaiid=" +
        pegawai;
    print(url);
  ...
  static Future getJadwalDokterByTglAndPoli(String date, String unitId) {
    var url = baseUrl +
        "/jadwaldokter/cari_by_tglunitid?tgl=" +
        date +
        "&unitid=" +
        unitId;
    return http.get(url);
  ...
  static Future getDataPasienByNoRm(noRm, tglLahir) {
    var url = baseUrl +
        "/pasien/cari_by_norm_tgllahir?norm=" +
        noRm +
        "&&tgllahir=" +
        tglLahir;
    return http.get(url);
  ...
  static Future getBookingByPasienAndStatus(
      idPasien, statusBooking, statusBooking2) {
    var url = baseUrl +
        "/Booking/get_booking_by_pasien/" +
        idPasien +
        "/" +
        statusBooking +
        "/" +
  ...
  static Future getBookingByPasienAndStatusBooking(idPasien, statusBooking) {
    var url = baseUrl +
        "booking/get_all_booking_by_status_booking_pasien/" +
        idPasien +
        "/" +
        statusBooking;
    return http.get(url);
  ...
  static Future getBookingByPendaftaranId(pendaftaranId) {
    var url =
        baseUrl + "/Booking/get_booking_by_pendaftaran_id/" + pendaftaranId;
    return http.get(url);
  }
  ...
  static Future getTotalAntrian(panggilan_status) {
    var url =
        baseUrl + "/antrian/get_total_antrian_by_pasien/" + panggilan_status;
    return http.get(url);
  }

  static Future getAntrianPanggil(panggilan_status, jadwal_praktek_id) {
    var url = baseUrl +
        "antrian/get_antrian_panggil_by_pasien/" +
        panggilan_status +
        '/' +
        jadwal_praktek_id;
    return http.get(url);
